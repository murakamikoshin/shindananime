/* 配る形を dist/ に組む。
       npm run build
   src/ をそのまま写し、Tailwind の CSS を書き出し、anime_data.json を添える。
...
if (!existsSync(join(root, 'anime_data.json'))) {
  console.error('anime_data.json が無い。先に python3 scrape_filmarks.py');
  process.exit(1);
}
...
cpSync(join(root, 'anime_data.json'), join(out, 'anime_data.json'));
...
const data = JSON.parse(readFileSync(join(out, 'anime_data.json'), 'utf8'));
console.log(`dist/: ${data.anime.length} 作（${data.meta.source}）`);
