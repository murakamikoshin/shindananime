...
全50問（基本20問＋好みの要素30問、続けて1回で聞く）・6軸であなたのアニメタイプ（例 `RDCP-VM`）と二つ名を割り出し、アニメのデータベース全体から
「運命の1作」と次点2作を選ぶ診断。
...
| `src/questions.js` | 質問（基本20問＋好みの要素30問、全50問） |
...
| 視覚・フェチズム | V 映像美・演出 | T ストーリー・脚本 | Q13–15 |
| 視聴スタイル | M 熟読・考察 | L 一気見・サクッと | Q16–18 |

- 回答は5段階（A / どちらかといえばA / どちらともいえない / どちらかといえばB / B）→ +1, +0.5, 0, -0.5, -1。軸ごとに平均
- **タイプコード**: 6軸の勝った側の文字を `[軸1][軸2][軸3][軸4]-[軸5][軸6]` に並べる（`RDCP-VM`、`FHSA-TL`）。ちょうど0なら + 側。どの軸も1文字（視覚だけ2文字の `ST` だったのを `T` に直した）
...
- **選び方**: 全作品とコサイン類似度を取り、`0.82×cos + quality + eraAdjust + prefAdjust` の合計（total）順に並べる。
  同じシリーズ（1期・2期・劇場版）と、軸がほぼ同じ作品は並べない
- **適合度**: `50 + 50×total`（%）。並び順と同じ total から出すので、表示される%は必ず高い順になる
  （前は cos だけから出していて「82%より80%が選ばれる」ように見えるバグがあった）
- `quality` は人気・★の小さな上乗せ（最大 0.126 = `0.084×人気 + 0.042×★`）。`eraAdjust`／`prefAdjust` も
  それぞれ小さな上乗せに抑えてある（大きすぎると、質問を増やしたり片方ばかり選んだ時に total が 1 を超えて、
  複数の作品が match 100% に張り付き、同じ系統でもニュアンスが違うはずの差が消える）
- **ローディングの「N件を除外」**: いちばんはっきり答えた2軸で、逆の側に寄っている作品を本当に数えた数。
  件数も DB の実際の件数を出す（1万件の DB を作れば「10,000件超」になる）

### 画質・年代と、好みの要素（タイプコードには入れない）

**全50問を続けて1回で聞く**（6軸×3問＋画質・年代2問＋好みの要素30問）。二段階に分けて「もっと絞り込む？」と
選ばせる作りはやめた。

- **画質・年代（Q19–20）**: 新しめ好きほど、古い作品を下げる（2000年より前は強く、2000年代は中くらい）。
  はっきり新しめを選んだ人は、結果画面の「放送年」がはじめから「2010年以降」になる（切り替えで外せる）
- **好みの要素（30問）**: 恋愛・グロ・異世界転生などの物語の中身（18問）に加えて、学園・ファンタジー・SF・
  ホラー・ミステリー・家族もの・お仕事ものなど舞台やジャンル寄りの要素（12問）。作品ごとに要素の強さ `f`（0〜1）を持たせ、
  - 好き（A）→ 1要素あたり最大 +0.15。6軸がよく合っている作品ほど強く効く（要素が軸を追い越さないため）。
    ただし好みの要素の合計は ±0.08 まで（`prefAdjust` の上限）
  - 苦手（B）→ 1要素あたり最大 -0.30。ほぼ避ける（こちらも合計 ±0.08 まで）
  - 「話題作 / 隠れた名作」だけは、作品の人気 `p` で上げ下げする

  | 分類 | 要素 |
  |---|---|
  | 物語の中身 | 恋愛 / ギャグ / 群像劇 / 泣ける / 日常・癒し |
  | 刺激の強さ | グロ・残酷 / お色気 |
  | ジャンル | 異世界転生 / スポーツ / ロボット / 音楽・アイドル |
  | 絵柄・作り | 萌え絵 / 3DCG / 子ども向け |
  | 長さ・形 | 長編 / 劇場版 / ショート |
  | 見つけ方 | 話題作 ↔ 隠れた名作 |

- 要素の強さは、手で付けたタグ（`data/default_titles.tsv`）と、あらすじ・レビューの言葉、媒体（劇場版）、
  Annict の話数（50話以上で長編）から出す（`build_anime_db.py` の `FEATURES`）
- **けなす言い方は数えない**（「作画崩壊」「泣けなかった」「グロくない」など。`NEG_PHRASES`）
- **映像**: Annict のスタッフ欄の制作スタジオが `data/visual_studios.tsv` にあれば V（映像美）を上げる。
  載せる会社は編集上の判断なので、好きに足し引きしてよい
...
- **解析中の枠の注意**: 中身が少なく自動で移る画面は、AdSense の「コンテンツの無い画面への広告」に当たる恐れがあったが、
  持ち主がリスクを承知の上で本物の広告を出す判断をした（2026-09-25）。`loadingPr` には切り替えない
- `ads.txt` は **koshinstudio.com の直下**に置く（koshin-studio リポジトリ）。広告を出し始めたら、
  koshin-studio の `privacy/index.html`（あつかい）の「広告について」も書き足す

### 配信ボタン（アフィリエイト）

- `config.js` の `vod` に12サービス（DMM TV・U-NEXT・Prime Video・Netflix・ABEMA・Hulu・Disney+・
  dアニメストア・TELASA・FOD・Lemino・バンダイチャンネル）を登録している
- どのボタンを出すかは `w.vod`（Filmarks が持っている「実際にここで見られる」データ）で絞り込む。
  `w.vod` が分からない作品だけ、今まで通り全部出す（`vodButtons`、`src/app.js`）
- `vod[].url` にアフィリエイトリンクを入れる。`{q}` に作品名、`{url}` にそのサービスの検索 URL が入る
  （例 A8.net: `https://px.a8.net/svt/ejp?a8mat=XXXX&a8ejpredirect={url}`）。今は全部未提携（`url: ''`）
- url を入れたボタンには **PR の札と `rel="sponsored"`** が付き、結果の下に「アフィリエイトリンクです」の注記が出る。
  ステマ規制（2023年10月〜）に沿うためなので、外さないこと
- url が空の間は各サービスの検索ページへのただのリンク。Disney+とバンダイチャンネルは検索URLの形が
  確かめられなかったので、トップページへのリンクになっている
- 「31日間無料」などの条件はサービス側で変わる。**提携先の最新の条件と `label` を合わせること**
- `w.vod` があってもFilmarks取得時点の情報なので、今も本当にそのサービスで見られるかは分からない
  （画面にも「各サービスでご確認ください」と出している）
...
- **★スコア**: 画面には一切出さない。`quality()`（軸との類似度に少しだけ足す内部計算）に使うだけの
  「あくまで診断材料」（2026-09-25）。Filmarks・Annict という名前も、リンクも画面には出ていない
- **あらすじ**: Filmarks の公式あらすじは権利者のもの。今は冒頭110字だけを出している
- **画像**: `config.js` の `showImages` は `true`。Annict（公式サイトの OGP 画像）を優先し、無ければ
  Filmarks（og:image）で補う（`build_anime_db.py` の `merge()`）。どちらも作品の権利者のものだが、
  元は他所での表示を前提にした販促画像で、持ち主がリスクを承知の上で出す判断をした（2026-09-25）
- 手元の約500作は**スコアを持たない**（数字をでっち上げないため）。画面には何も出さない
