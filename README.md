...
基本20問（＋精度アップの18問）・6軸であなたのアニメタイプ（例 `RDCP-VM`）と二つ名を割り出し、アニメのデータベース全体から
「運命の1作」と次点2作を選ぶ診断。
...
| `src/questions.js` | 質問（基本20問＋追加18問） |
...
**基本の20問**（6軸×3問＋画質・年代2問）のあと、「結果を見る」か「追加の18問で精度を上げる」かを選べる（全38問）。
結果画面からも、答えをそのまま追加の問題に進める。

- **画質・年代（Q19–20）**: 新しめ好きほど、古い作品を下げる（2000年より前は強く、2000年代は中くらい）。
  はっきり新しめを選んだ人は、結果画面の「放送年」がはじめから「2010年以降」になる（切り替えで外せる）
- **好みの要素（追加の18問）**: 作品ごとに要素の強さ `f`（0〜1）を持たせ、
  - 好き（A）→ 最大 +0.15。6軸がよく合っている作品ほど強く効く（要素が軸を追い越さないため）
  - 苦手（B）→ 最大 -0.30。ほぼ避ける
