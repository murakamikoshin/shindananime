...
- バグ: タイトルが長くて3行以上になる作品カードで、絶対配置の「次点」「◯◯%適合」バッジにタイトルがかぶって隠れることがあった。`top` の高さを固定（`h-56`/`h-32`）から最低高さ（`min-h-56`/`min-h-32`）に変え、さらにタイトルの `<h3>` に `mt-12` を付けてバッジの下に必ず隙間ができるようにした。実データの最長タイトル（98文字、8行）で確認済み（2026-09-25、`src/app.js` の `card`）
- 「NG騎士ラムネ&40」に画像が無いのは不具合ではない → 調べたら Annict・Filmarks どちらにも実物の画像が無く、Filmarks 側は `placehold`（仮絵）のみで、既存の `BAD_IMAGE_HINTS` が正しく弾いている状態だった（2026-09-25）
